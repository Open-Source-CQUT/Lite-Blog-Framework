package com.liteweb.modules.common.exception.lang;
...
public enum LiteBlogExceptionStatus {
    ...
    PASSWORD_CHANGE_OK(HttpStatus.OK.value(), "密码修改成功!", HttpStatus.Series.SUCCESSFUL),

    LOGOUT_OK(HttpStatus.OK.value(), "用户注销成功!", HttpStatus.Series.SUCCESSFUL),
    ...
    PASSWORD_RE_ERROR(HttpStatus.BAD_REQUEST.value(), "两次输入的密码不一致!", HttpStatus.Series.CLIENT_ERROR),
    ...
    //500
    REGISTER_FAIL(HttpStatus.INTERNAL_SERVER_ERROR.value(), "用户注册失败!", HttpStatus.Series.SERVER_ERROR),

    LOGOUT_FAIL(HttpStatus.INTERNAL_SERVER_ERROR.value(), "用户注销失败!", HttpStatus.Series.SERVER_ERROR),
    PASSWORD_CHANGE_FAIL(HttpStatus.INTERNAL_SERVER_ERROR.value(), "密码修改失败!", HttpStatus.Series.SERVER_ERROR);
